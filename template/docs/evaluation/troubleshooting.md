...
## Bedrock / Claude cannot be the stock judge

NeMo's built-in judge client always sends **both** `temperature` and `top_p`. The DR gateway's **Bedrock** models (for example `bedrock/anthropic.claude-sonnet-4-6`) reject that combination:
