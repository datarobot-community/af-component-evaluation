...
Each case is an attack that tries to hijack the agent into emitting a known "canary" string. If the canary shows up in the response, the injection succeeded. Checking for a fixed string is objective and un-foolable.
