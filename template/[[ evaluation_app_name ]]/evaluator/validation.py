...
def preflight_judge(judge_cfg: dict[str, Any]) -> None:
    ...
    # Reasoning models (gpt-5-5, o-series, etc.) spend tokens on an internal
    # reasoning pass before emitting output, so a tight max_tokens budget yields
    # an HTTP 400 ("max_tokens reached") even when auth and model are fine —
    # a false negative for the preflight. Give the ping enough headroom to
    # finish reasoning and emit at least one output token.
    payload = json.dumps(
        {
            "model": model_id,
