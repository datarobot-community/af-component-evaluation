...
- Run `task eval` / `run.py`, or interpret `eval_results.json` / `eval_status.json`
...
The evaluation component folder is named during setup (e.g. `evaluations/`). Find it
by scanning for the `[tool.af-component]` marker — never assume the folder name:

```bash
grep -rl 'type = "evaluation"' --include=pyproject.toml .
```

`cd` into that directory. All commands below run from inside the component.
...
2. **From a spreadsheet** — convert a CSV (`id, source, input` required):
   ```bash
   task convert -- user_datasets/my_cases.csv
   ```
3. **Synthetic** — generate a good/bad mix with Claude from a description of the agent:
   ```bash
   task generate -- \
...
|---|:---:|---|
| `DATAROBOT_API_TOKEN` | yes | Bearer token for the DR LLM gateway (judge runs and `task generate`) |
...
> ⚠️ **Judge model gotcha:** use an **Azure** GPT judge (e.g. `azure/gpt-5-5-2026-04-230`),
...
- **Tools:** `uv`, `task` (go-task), and a running agent endpoint.
- **Env vars** (in the project-root `.env`, one level above the component folder):
  `DATAROBOT_API_TOKEN` and `DATAROBOT_ENDPOINT` (judge runs and `task generate`),
